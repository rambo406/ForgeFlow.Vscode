...
export interface MonacoDiffCommentThread {
  ...
  export class MonacoDiffViewerComponent implements AfterViewInit, OnDestroy, OnChanges {
    ...
    // Legacy PR comment threads (left/right side threads)
    @Input() comments: MonacoDiffCommentThread[] = [];
    // Inline AI comments (lighter-weight)
    ...
    constructor(private readonly bus: WebviewMessagingService) {
      // Listen for suggest result updates to populate open widget if present
      this.messageSub = this.bus.onMessage().subscribe((msg: any) => {
        try {
          ...
          // id format: ff-comment-thread-<side>-<line>
          const side = parts[3];
    ...
    private render(): void {
      ...
      // Apply legacy PR comment threads
      this.applyComments();
      // Wire click handlers for comment glyphs
      this.installClickHandlers();
      // Apply glyph decorations for changed lines and wire analyze clicks
      this.applyChangedLineGlyphs();
      try { this.editor.onDidUpdateDiff?.(() => this.applyChangedLineGlyphs()); } catch { /* noop */ }

      // Wire glyph click handler on modified editor
      ...
      // Apply any inline AI comments
      this.applyInlineComments();

      // Apply pending reveal if provided
    ...
    private applyComments(): void {
      ...
      try {
        if (original?.deltaDecorations) this.decorationsOriginal = original.deltaDecorations(this.decorationsOriginal, leftDecs);
        if (modified?.deltaDecorations) this.decorationsModified = modified.deltaDecorations(this.decorationsModified, rightDecs);
      } catch { /* noop */ }
    }
    ...
    private installClickHandlers(): void {
      ...
      try { original?.onMouseDown?.(onMouse('left')); } catch { /* noop */ }
      try { modified?.onMouseDown?.(onMouse('right')); } catch { /* noop */ }
    }
    ...
    // --- AI glyphs / inline comment support ---
